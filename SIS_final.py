...
def create_csv():
    ...
    try:  # put csv file into a list of row
        ...
        return headers, []
    # Return the headers and the list of rows
    if not rows:
        return headers, []
    else:
...
def updateStudent(ID_Number):
    # Search for the ID_Number in the Treeview widget
    for row_id in student_table.get_children():
        if student_table.item(row_id, 'values')[0] == ID_Number:
            ...
            # Fill up information in the pop-up window
            id_label = Label(popup_window, text="ID_Number:")
            ...
            def submit_update():
                ...
                # Update the status based on course existence
                if course_exists:
            ...
            return  # Exit the function after finding and displaying the data

    # If the ID_Number is not found, show an error message
...
def findIDNumber():
    ...
    # Find the row with the corresponding ID_Number
    found = False
    for row_id in student_table.get_children():
        values = student_table.item(row_id, 'values')
    ...
    # Open the update popup window with the selected ID_Number
...
def deleteStudent():
    ...
    # If delete button is pressed without selecting a row from the table, print an error
    ...
    # If confirmed, delete from table and from CSV
    if confirm_deletion:
...
def save_to_csv():
    # Get all the data from the Treeview widget
...
def addCourse():
    ...
    #another function to finalize the submission when submit button is clicked.
    def submit_course():
        ...
        if course_code and course_name:  # Check if both fields are filled
            ...
            popup_window.destroy()  # Close the pop-up window after submission
        else:
            # Show an error message if any field is empty
            messagebox.showerror("Error", "Both Course Code and Course Name are required.")

    # Button to submit course information
...
def update_status(course_code): #update student status in case there's a new course code added
    # Iterate through all rows in the student table
    for row_id in student_table.get_children():
        values = student_table.item(row_id, 'values')
    ...
    if course_name:  # check course name
        ...
        for row in course_table.get_children():
            ...
            if current_course_name.lower() == course_name_lowercase:
                ...
                return
        # If course name is not found, show an error message
...
def delete_course_from_csv(course_name):
    ...
    if course_code:
        ...
        for row_id in student_table.get_children():
            ...
            if values[4] == course_code:
                # Update the status to "NOT ENROLLED"
