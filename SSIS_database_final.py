...
def edit_course():
    ...
    # Fetch the current information of the selected course from the database
    conn = sqlite3.connect('student.db')
    cursor = conn.cursor()
    cursor.execute("SELECT Course_Code, Course_Name FROM course WHERE Course_Name = ?", (course_name,))
    ...
    # Create a popup window for editing course information
    popup_window = Toplevel(root)
    popup_window.geometry("300x150")
    popup_window.title("Edit Course")

    # Labels and entry fields for course information
    course_code_label = Label(popup_window, text="Course Code:")
    course_code_label.grid(row=0, column=0, padx=5, pady=5)
    course_code_entry = Entry(popup_window)
    course_code_entry.grid(row=0, column=1, padx=5, pady=5)
    course_code_entry.insert(0, course_data[0])

    course_name_label = Label(popup_window, text="Course Name:")
    course_name_label.grid(row=1, column=0, padx=5, pady=5)
    course_name_entry = Entry(popup_window)
    course_name_entry.grid(row=1, column=1, padx=5, pady=5)
    course_name_entry.insert(0, course_data[1])

    # Function to update the course information
    def update_course():
        ...
        if new_course_code and new_course_name:
            ...
            # Check if the new course code already exists in the database
            cursor.execute("SELECT COUNT(*) FROM course WHERE Course_Code = ?", (new_course_code,))
            existing_course = cursor.fetchone()[0]

            if existing_course > 0 and new_course_code != course_data[0]:
                messagebox.showerror("Error", f"Course code '{new_course_code}' already exists.")
            else:
                # Update the course information in the database
                cursor.execute("UPDATE course SET Course_Code = ?, Course_Name = ? WHERE Course_Code = ?",
                               (new_course_code, new_course_name, course_data[0]))
                conn.commit()

                # Update the displayed course information in the GUI
                course_table.item(selected_item, values=(new_course_name, new_course_code))

                # Check if any students are enrolled in the old course
                cursor.execute("SELECT ID_Number FROM student WHERE Course_Code = ?", (course_data[0],))
                students_enrolled = cursor.fetchall()

                # Update student enrollments
                for id_number in students_enrolled:
                    cursor.execute("UPDATE student SET Course_Code = '', Status = 'NOT ENROLLED' WHERE ID_Number = ? AND Course_Code = ?", (id_number[0], course_data[0]))
    ...
    update_button = Button(popup_window, text="Update", command=update_course)
...
search_bar.grid(row=8, column=0, sticky="n", pady = (20, 0))
...
edit_course_button =  Button(frame, text = "Edit Course", width = 10, command = edit_course, fg = "black", bg= "#FFF6E0", font=("David", 14, "bold"))
...
edit_course_button.grid(row = 5, column = 0)
delete_course_button.grid(row = 6, column = 0)
format_label.grid(row = 10, column = 0 , pady = 20)
display_student_data()
display_course_data()
